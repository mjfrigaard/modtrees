...
launch_app <- function(app = NULL, options = list(), run = "p", ...) {
  ...
  tryCatch({
    ...
    if (app == "bslib") {
      ...
      shinyAppDir(
        appDir = system.file("tidy-movies", package = "modtrees"),
        options = options
      )
    } else if (app == "quarto") {
      quarto::quarto_preview(
        system.file("quarto", "index.qmd", package = "modtrees"), 
        render = "all"
      )
    } else {
      # Resolve the app directory: use system.file when installed,
      # otherwise fall back to inst/app/ relative to the project root
